...
const AdminReports = () => {
  const handleExport = () => {
    const csvContent = "Tháng,Yêu cầu,Hợp đồng\n" + conversionData.map(d => `${d.month},${d.requests},${d.contracts}`).join("\n");
    const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `bao-cao-eternal-${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-surface-lowest rounded-xl p-6 shadow-ambient">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <PieChart><Pie data={revenueByType} dataKey="value" cx="50%" cy="50%" outerRadius={80} innerRadius={50}>{revenueByType.map((entry, i) => <Cell key={i} fill={entry.color} />)}</Pie><Tooltip /></PieChart>
          </ResponsiveContainer>

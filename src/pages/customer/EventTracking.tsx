...
import { CheckCircle, Circle, Clock, MessageSquare, FileText, CreditCard, ArrowLeft, Send, Upload } from "lucide-react";
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto pb-2">
          {[
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-body text-sm whitespace-nowrap transition-all ${
                activeTab === tab.key ? "gradient-primary text-primary-foreground" : "bg-surface-lowest text-muted-foreground hover:text-foreground"
              }`}
            >
              ...
              {milestones.map((milestone, i) => (
                <motion.div key={milestone.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="relative flex items-start gap-6 mb-8">
                  <div className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
                    milestone.status === "done" ? "bg-secondary text-secondary-foreground" :
                    milestone.status === "current" ? "gradient-primary text-primary-foreground animate-pulse" :
                    "bg-surface-high text-muted-foreground"
                  }`}>
                    {milestone.status === "done" ? <CheckCircle size={18} /> :
                     milestone.status === "current" ? <Clock size={18} /> :
                     <Circle size={18} />}
                  </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-3xl">
            <div className="bg-surface-lowest rounded-xl shadow-ambient overflow-hidden">
              ...
              <div className="p-6 space-y-4 max-h-96 overflow-y-auto">
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex ${msg.isManager ? "justify-start" : "justify-end"}`}>
              ...
              <div className="p-4 bg-surface-low flex gap-3">
                <Input value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Nhập tin nhắn..." className="flex-1 rounded-xl bg-surface-lowest font-body border-none" />
                <Button variant="ghost" size="icon"><Upload size={18} /></Button>
                <Button variant="hero" size="icon"><Send size={18} /></Button>
              </div>
            </div>
          </motion.div>
        )}

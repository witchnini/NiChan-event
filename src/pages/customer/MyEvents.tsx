      ...
      <section className="py-12">
        <div className="container mx-auto px-6">
          ...
          <div className="flex gap-3 mb-8 flex-wrap">
            {["Tất cả", "Đang chuẩn bị", "Đã báo giá", "Hoàn thành"].map((tab) => (
              <button key={tab} className={`px-4 py-2 rounded-xl font-body text-sm transition-all ${tab === "Tất cả" ? "gradient-primary text-primary-foreground" : "bg-surface-lowest text-muted-foreground hover:text-foreground"}`}>
                {tab}
              </button>
            ))}
          </div>
